...
2. **出力は2経路を可視状態で切り替える**（`setupAudioGraph` + `applyOutputRouting`）
   — フォアグラウンド: `ctx.destination` 直結 / バックグラウンド: MediaStream →
   `<audio>` 要素経由。iOS は Web Audio の直接出力をバックグラウンドで停止するが、
   メディア要素の再生は継続するため、背面では stream 経由が必須（直結のみに戻すと
   ホーム画面移動で音が止まる回帰）。逆に**常時 stream 経由にすると、メディア要素の
   ストール後の追いつき再生（時間圧縮）でフォアグラウンドの「タタタタ」連打が再発**する。
   両経路とも常に信号を流し、GainNode の 0/1 だけで切り替えている
